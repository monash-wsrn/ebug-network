...
from romi_msg.msg import RomiIn
...
class Poller(Node):
    ...
    def __init__(self):
        ...
        self.cam_err = self.create_subscription(Log, 'robot_'+str(self.robot_id)+'/rosout/cam', self.error_management, 10)
        ...
        self.dir_sub = self.create_subscription(
            RomiIn,
            'robot_'+str(self.robot_id)+'/status',
            self.poll_dir,
            10
        )
        ...
        self.redirect_rosout = self.create_publisher(Log, 'rosout', 10)
    ...
    def error_management(self, msg):


        if msg.level == 40:
            self.run = 0
            cam_id = int(msg.name[-1])
            print('error management')
            self.redirect_rosout.publish(msg)
            off = self.cli_list[cam_id].call_async(self.off_req)
            self.get_logger().info('Error management')



    def poll_dir(self, msg):
