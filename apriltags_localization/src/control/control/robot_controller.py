...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.timer = self.create_timer(0.01, self.odom_pose_update)

        self.odom_pub=self.create_publisher(Odometry, 'robot_'+self.robot_id+'/odometry', 10)
    ...
    def odom_pose_update(self):
        ...
        if self.start: # first value
            ...
            q = quaternion_from_euler(.0, .0, self.odom_th)
    ...
    def robot_controller(self, msg:Odometry):
        ...
        th = np.arctan2((PATH[self.path_idx][1]-msg.pose.pose.position.y),(PATH[self.path_idx][0]-msg.pose.pose.position.x))
        v,w = self.planner.plan(PATH[self.path_idx][0], PATH[self.path_idx][1], th, msg.pose.pose.position.x, msg.pose.pose.position.y, euler[2])
        ...
        if not self.done:
            if v == 0.0 and w==0.0:
                ...
                duty_cycle_l, duty_cycle_r = self.drive(v,w,self.wl, self.wr)
                self.motors(duty_cycle_l, duty_cycle_r)
