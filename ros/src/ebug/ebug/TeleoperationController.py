...
DRIFT_CORRECTION = 1.118  # Adjusted drift correction factor
...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.create_timer(0.1, self.publish_odometry)  # Publish odometry at 10 Hz

    def motors(self, left, right):
        self.try_i2c(lambda : self.a_star.motors(int(left), int(right)), "I/O error moving motors")
        self.try_i2c(lambda : self.a_star.led_ring(int(0), int(0), int(255)), "I/O error setting LEDs")  # Blue color
    ...
    def drive(self, v_desired, w_desired):
        ...
        # Apply wheel-specific corrections
        wl_desired *= LEFT_WHEEL_CORRECTION
        wr_desired *= RIGHT_WHEEL_CORRECTION
