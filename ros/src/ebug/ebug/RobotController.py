...
class RobotController(Node):
    ...
    def __init__(self):
        ...
        self.max_retry_i2c = int(os.getenv('I2C_RETRIES', "256"))
        self.i2c_frequency = float(os.getenv('I2C_FREQUENCY', "50.0"))
    ...
    def i2c_update(self):
        ...
        # Debug: Print encoder readings and desired velocities
        self.get_logger().info(f"Encoder Left: {self.lenc_current}, Right: {self.renc_current}")
        self.get_logger().info(f"Desired Left: {self.lenc_desired}, Right: {self.renc_desired}")
    ...
    def gyroscope(self):
        on_error = lambda : self.get_logger().info("I/O error reading from gyroscope")
        result = None
    ...
    def update_odom(self):
        ...
        if abs(dl - dr) < 1e-6:
            ...
            nyaw = math.remainder(yaw + dw, math.tau)  # Bound angle to [-pi, pi]
            self.odom = (nx, ny, nyaw)
    ...
    def control_callback(self, msg:ControlCommand):
        ...
        # Debug: Print control command values
        self.get_logger().info(f"Control Command - Linear: {msg.control.linear.x}, Angular: {msg
